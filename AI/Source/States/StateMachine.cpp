...
StateMachine::~StateMachine()
{
	for (std::unordered_map<std::string, State*>::iterator it = m_stateMap.begin(); it != m_stateMap.end(); ++it)
	{
		delete it->second;
	}
	m_stateMap.clear();
...
{
	std::unordered_map<std::string, State*>::iterator it = m_stateMap.find(nextStateID);

	if (it != m_stateMap.end())
	{
		m_nextState = static_cast<State*>(it->second);
	}
}
...
void StateMachine::UpdateTurn()
{
	if (m_nextState == m_currState)
	{
		m_currState->UpdateTurn();
	}
}
